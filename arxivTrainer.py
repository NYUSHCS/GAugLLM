...
from arxivModel import CustomXTransformer
...
class GiantTrainer_new():
    def __init__(self,data,text,filename,device):
        ...
        self.device = device
        self.max_deg = 1000000000
        print("text length",len(text))
        print("data.x length",len(data.x))
        print("data.y length",len(data.y))
        ...
        self.checkpoints_dir = f'checkpoints/ogbn-arxiv/'
        ...
        with open('/LLM4GCL_update/LLMs/prt_lm/ogbn-arxiv/Aug_degrees_list.json', 'w') as f:
