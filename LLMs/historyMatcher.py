...
from pecos.xmc.xtransformer.module import XMCLabelTensorizer, XMCTextTensorizer, XMCTextDataset
...
class SimilarityAttentionMLP(nn.Module):
    def __init__(self, emb_size):
        super(SimilarityAttentionMLP, self).__init__()
        ...
        self.fc2 = nn.Linear(4 * hidden_size, 4)  # 输出4个权重

    def forward(self, embs_list):
        new_embs_list = []
        similaritys = []
        content_emb = [embs_list[i] for i in [0,2,4,6]]
        context_emb = [embs_list[i] for i in [1,3,5,7]]

        # Step 1: 处理 content_emb 并计算初步权重
        for embs in content_emb:
            embs = self.fc1(embs)
            embs = self.relu(embs)
            new_embs_list.append(embs)
        x = torch.cat(new_embs_list, dim=1).to("cuda")
        content_weights = self.fc2(x)

        # Step 2: 计算相似度
        for i in range(4):
            # 相似度计算，可以选择点积或余弦相似度
            similarity = (content_emb[i] * context_emb[i]).sum(dim=1, keepdim=True)
            # 或者 similarity = F.cosine_similarity(content_emb[i], context_emb[i], dim=1, eps=1e-6).unsqueeze(1)
            similaritys.append(similarity)

        # Step 3: 合并权重与相似度
        similaritys = torch.cat(similaritys, dim=1)
        weights = content_weights + similaritys
        ...
        weighted_embs = weights * torch.stack(content_emb, dim=1)
        weighted_embs = weighted_embs.sum(dim=1)

        return weights.squeeze(-1), weighted_embs


class ConcatAttentionMLP(nn.Module):
    def __init__(self, emb_size):
        super(ConcatAttentionMLP, self).__init__()
        self.temperature = 0.01
        self.emb_size = emb_size
        input_size = 2 * emb_size  # 因为输入是2个拼接的tensor
        hidden_size = int(input_size * 0.5)
        self.fc1 = nn.Linear(input_size, hidden_size)
        self.relu = nn.LeakyReLU()
        self.fc2 = nn.Linear(4 * hidden_size, 4)  # 输出4个权重
    ...
    def forward(self, embs_list):
        content_context_embs = [torch.cat((embs_list[i], embs_list[i+1]), dim=1) for i in range(0, len(embs_list), 2)]
        new_embs_list = []
        content_embs = [embs_list[i] for i in [0,2,4,6]]
        for embs in content_context_embs:
            embs = self.fc1(embs)
            embs = self.relu(embs)
            new_embs_list.append(embs)
        x  = torch.cat(new_embs_list, dim=1).to("cuda")
        weights = self.fc2(x) / self.temperature
        weights = F.softmax(weights, dim=1)  # 确保权重和为1

        # 计算加权平均的 logits
        # 权重的维度调整：从 [batch_size, 4] 变为 [batch_size, 4, 1]
        weights = weights.unsqueeze(-1)
        # 计算加权平均的 logits
        content_embs = torch.cat(content_embs, dim=1).to("cuda").reshape(-1, 4, self.emb_size)
        weighted_embs = weights * content_embs  # 对X中的每个元素进行加权
        weighted_embs = weighted_embs.sum(dim=1)  # 对第二个维度求和以得到加权平均
        return weights.squeeze(-1), weighted_embs
...
class AttentionMLP(nn.Module):
    ...
    def forward(self, embs_list):
        new_embs_list = []
        first_half_embs = [tensor[:, :self.emb_size] for tensor in embs_list]
        first_half_embs = torch.cat(first_half_embs, dim=1).to("cuda")
        first_half_embs = first_half_embs.reshape(-1, 4, self.emb_size)
        ...
        weights = weights.unsqueeze(-1)
        # 计算加权平均的 logits
        weighted_embs = weights * first_half_embs  # 对X中的每个元素进行加权
        weighted_embs = weighted_embs.sum(dim=1)  # 对第二个维度求和以得到加权平均
        return weights.squeeze(-1), weighted_embs
    
class CusTransformerMatcher(pecos.BaseClass):
    ...
    def __init__(
        ...
        if attention_type == "concat":
            self.mlp = ConcatAttentionMLP(emb_size=768)
        elif attention_type == "similarity":
            self.mlp = SimilarityAttentionMLP(emb_size=768)
        self.weights = torch.randn(19717, 4)
        self.weights_path = "/LLM4GCL_update/LLMs/prt_lm/amazon-history/training_weight.emb"
        self.checkpoint_dir = "/LLM4GCL_update/LLMs/checkpoints/amazon-history/"
    ...
    def predict(
        ...
        **kwargs,
        ...
        for X_text in X_text_list:
            ...
            elif not isinstance(pred_params, CusTransformerMatcher.PredParams):
        ...
        if epoch_index == 0:
            weights = torch.load(self.weights_path).to("cuda")
            print("During Training: Weights Used For Embeddings",weights)
            number_of_nodes = embs[0].shape[0]
            #加权pooling
            final_embeddings = torch.zeros(number_of_nodes, 768).to("cuda")
            for i, emb in enumerate(embs):
                # 将 numpy.ndarray 转换为 Tensor
                emb_tensor = torch.tensor(emb, dtype=torch.float32).to("cuda")

                # 调整权重维度以匹配 emb 并进行加权
                weighted_emb = emb_tensor * weights[:, i].unsqueeze(1).to("cuda")
                final_embeddings += weighted_emb

            final_embeddings_np = final_embeddings.detach().cpu().numpy()

        if epoch_index != 0:
            weights_path = f"/LLM4GCL_update/LLMs/checkpoints/amazon-history/{epoch_index}/{self.attention_type}-weights.emb"
            weights = torch.load(weights_path).to("cuda")
            print("During Eval: Weights For Embeddings Loaded",weights)
            ...
            save_path = f"/LLM4GCL_update/LLMs/checkpoints/amazon-history/{epoch_index}/{self.attention_type}-embs.emb"
            print("Start to save Attention Embeddings")
            torch.save(final_embeddings, save_path)
            print(f"Epoch {epoch_index} Attention Embeddings Saved!")
            print("Final Embeddings:",final_embeddings)
            final_embeddings_np = final_embeddings.detach().cpu().numpy()
        return label_pred, final_embeddings_np
        ...
        if not only_embeddings:
            ...
            if all(v is not None for v in [self.concat_model, X_feat]):
                concat_pred_csr_codes = self.concat_model.predict(
                    CusTransformerMatcher.concat_features(X_feat, embeddings, normalize_emb=True),
                ...
                pred_csr_codes = CusTransformerMatcher.ensemble_prediction(
    ...
    def fine_tune_encoder(self, probs, val_prob=None, val_csr_codes=None, save_embedding_for_each_epoch = False):
        ...
        loss_function = CusTransformerMatcher.get_loss_function(train_params.loss_function).to(
            self.device
        )
        ...
        print("probs[0] text[0]",probs[0].X_text[0])
        print("probs[1] text[0]",probs[1].X_text[0])
        #print("probs[4] text[0]",probs[4].X_text[0])

        train_datas = [self.prepare_data(
            prob,
            label_padding_idx=self.text_model.label_padding_idx,
        ...
        # since number of active labels may vary
        # using pinned memory will slow down data loading
        fix_sampler_for_each_prob = FixedRandomSampler(train_data)
        train_dataloaders = [DataLoader(
            train_data,
            sampler=fix_sampler_for_each_prob,
            pin_memory=False,
        ...
        mlp_parameters = list(self.mlp.parameters())
        mlp_optimizer = AdamW(
            mlp_parameters,
            lr=5e-6,
            eps=train_params.adam_epsilon,
        )
        mlp_scheduler = get_scheduler(
            train_params.lr_schedule,
            emb_optimizer,
            num_warmup_steps=train_params.warmup_steps,
            num_training_steps=t_total,
        ...
        for epoch in range(1, int(train_params.num_train_epochs) + 1):
            print("Epoch ", epoch)
            if epoch == int(train_params.num_train_epochs):
                print("Last Epoch")
            #weights = torch.empty((0, 4), device="cuda") 
            new_weights = torch.empty((0, 4), device="cuda") 
            #for batch_cnt, batch in enumerate(train_dataloader):
            
            for batch_cnt, batches in enumerate(tqdm(zip(*train_dataloaders), total=len(train_dataloaders[0]))):
                all_logits = []
                all_cls_embs = []
                count = 0
                ...
                for batch_index,batch in enumerate(batches):
                    ...
                    cls_embs = outputs["pooled_output"]
                    logits = outputs["logits"]
                    all_logits.append(logits)
                    all_cls_embs.append(cls_embs)
                    inputs_label_values = inputs["label_values"]
                    label_embedding = (text_model_W_seq, text_model_b_seq)
                    count += 1
                self.text_encoder.train()
                self.text_model.train()
                print(f"text[0] embs for batch{batch_cnt}",all_cls_embs[0])

                weights_this_batch, weights_cls_embs = self.mlp(all_cls_embs)
                if label_embedding is not None:
                    W_act, b_act = label_embedding
                    W_act = W_act.to(weights_cls_embs.device)
                    b_act = b_act.to(weights_cls_embs.device)
                    logits = (weights_cls_embs.unsqueeze(1) * W_act).sum(dim=-1) + b_act.squeeze(2)
                    print("logits shape",logits.shape)
                new_weights = torch.cat((new_weights, weights_this_batch), dim=0)
                loss = loss_function(logits, inputs_label_values.to(self.device))
                loss = loss.mean()  # mean() to average on multi-gpu parallel training

                if train_params.gradient_accumulation_steps > 1:
                    loss = loss / train_params.gradient_accumulation_steps
                print(f"loss for batch: {batch_cnt}:",loss.item())
                loss.backward()
                ...
                if (batch_cnt + 1) % train_params.gradient_accumulation_steps == 0:
                    ...
                    if train_params.save_steps > 0 and global_step % train_params.save_steps == 0:
                        ...
                        if save_cur_model:
                            ...
                            print("model dir:",train_params.checkpoint_dir)
            ...
            if new_weights.shape == torch.Size([41551, 4]):
                self.weights = new_weights
                print("update self.weights after a whole epoch", self.weights)
                torch.save(self.weights,self.weights_path)
                print("training weights saved")

            
            if save_embedding_for_each_epoch:
                temp_checkpoint_dir = self.checkpoint_dir+ f"{epoch}/"
                print(f"Last round epoch {epoch} model dir:",temp_checkpoint_dir)
                self.save(temp_checkpoint_dir)
                print("Model CheckPoint Saved!")
                temp_weights_dir = self.checkpoint_dir+ f"{epoch}/{self.attention_type}-weights.emb"
                print(f"Saving weights to path for last round epoch: {epoch}")
                torch.save(self.weights,temp_weights_dir)
    ...
    def train(
        ...
        train_params=None,
        pred_params=None,
        save_embedding_for_each_epoch = False,
        **kwargs,
    ):
        ...
        print(probs[0].X_text[0])
        print(probs[1].X_text[0])
        ...
        if train_params.max_steps > 0 or train_params.num_train_epochs > 0:
            ...
            print(probs[0].X_text[0].ids)
            print(probs[1].X_text[0].ids)
            #print(probs[4].X_text[0].ids)
            matcher.fine_tune_encoder(probs, val_prob=None, val_csr_codes=val_csr_codes, save_embedding_for_each_epoch = save_embedding_for_each_epoch)
            if os.path.exists(train_params.checkpoint_dir):
                LOGGER.info(
                    "Reload the best checkpoint from {}".format(train_params.checkpoint_dir)
                )
                matcher = CusTransformerMatcher.load(train_params.checkpoint_dir)
                matcher.to_device(device, n_gpu)
        ...
        if train_concat or return_pred_on_trn or return_embed_on_trn:
            X_test_list = [probs[i].X_text for i in [0,2,4,6]]
        ...
        if train_concat:
            ...
            cat_embeddings = CusTransformerMatcher.concat_features(
                prob.X_feat,
                inst_embeddings,
                normalize_emb=True,
            )
            ...
            P_trn = CusTransformerMatcher.ensemble_prediction(
                P_trn,
                concat_P_trn,
