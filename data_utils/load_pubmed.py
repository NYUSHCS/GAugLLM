...
def add_generate_text_pubmed(text):
    # read GPT generated texts
    directory = "../dataset/generated_texts/pubmed"
    ...
    for i, file in enumerate(files):
        if file.endswith('.txt'):
            with open(os.path.join(directory, file), 'r', encoding='utf-8') as f:
                file_content = f.read().strip()  # This reads the entire content of the file
                text[i] = text[i] + file_content
                if i==0:
                    print(text[0])

def add_summery_text_pubmed(text):
    # read GPT generated texts
    directory = "../dataset/generated_summery/pubmed"
    files = os.listdir(directory)
    files = natsorted(files)
    assert len(text) == len(files), "text length: {}, files length: {}".format(len(text), len(files))
    for i, file in enumerate(files):
